...
static void renderInputBox(SDL_Renderer* rr, TTF_Font* fLabel, TTF_Font* fVal,
    ...
    SDL_SetRenderDrawColor(rr,
        box->active ? 255 : 248,
        box->active ? 240 : 248,
        box->active ? 240 : 255, 255);
    SDL_RenderFillRect(rr, &box->rect);

    /* border - blue when active, grey-blue when not */
    SDL_SetRenderDrawColor(rr,
        box->active ? 20  : 100,
        box->active ? 60  : 120,
        box->active ? 200 : 200, 255);
    ...
    if (strlen(box->value) > 0)
        renderText(rr, fVal, box->value,
                   ...
                   (SDL_Color){20, 30, 80, 255});
}

static void renderButton(SDL_Renderer* rr, TTF_Font* f, Button* btn) {
    SDL_Color bg, shadow;
    if (btn->isBlue) {
        bg     = btn->clicked ? (SDL_Color){ 10, 30,120,255}
               : btn->hovered ? (SDL_Color){ 30, 80,210,255}
                              : (SDL_Color){ 20, 60,180,255};
        shadow = (SDL_Color){5, 15, 70, 255};
    } else {
        bg     = btn->clicked ? (SDL_Color){110, 15, 15,255}
               : btn->hovered ? (SDL_Color){210, 50, 50,255}
                              : (SDL_Color){170, 25, 25,255};
        shadow = (SDL_Color){60, 8, 8, 255};
    }
...
#define HOVER(b) ((b).hovered=(mx>=(b).rect.x&&mx<(b).rect.x+(b).rect.w&&\
        ...
        /* -------- LANDING -------- */
        if (screen==SCREEN_LANDING) {
            /* gradient background: cream -> light blue */
            for (int i=0;i<WIN_H;i++) {
                float t=(float)i/WIN_H;
                SDL_SetRenderDrawColor(renderer,
                    (int)(250-t*20),(int)(250-t*22),(int)(255-t*10),255);
            ...
            /* ---- SOLVER CARD ---- */
            {
                ...
                /* blue left accent */
                SDL_SetRenderDrawColor(renderer,20,55,200,255);
                ...
                SDL_SetRenderDrawColor(renderer,25,65,205,255);
            ...
            /* ---- ABOUT CARD ---- */
            {
                SDL_Color bg = hoverAbout
                    ? (SDL_Color){255,222,222,255}
                    : (SDL_Color){255,234,234,255};

                SDL_SetRenderDrawColor(renderer,160,50,50,120);
                ...
                /* red left accent */
                SDL_SetRenderDrawColor(renderer,195,18,18,255);
                SDL_Rect rb2={cardAbout.x,cardAbout.y,6,cardAbout.h};
                SDL_RenderFillRect(renderer,&rb2);
                SDL_SetRenderDrawColor(renderer,210,110,110,255);
            ...
            renderCenterText(renderer,fNorm,
                ...
                cx,WIN_H-34,(SDL_Color){200,215,255,255});
        }
        ...
        /* -------- ABOUT -------- */
            ...
            renderCenterBold(renderer,fHuge,"ABOUT US",cx,50,
                             (SDL_Color){10,30,130,255});

            /* two-tone underline */
            SDL_SetRenderDrawColor(renderer,20,60,210,255);
            ...
            drawPanel(renderer,230,112,1140,750,
                      (SDL_Color){250,252,255,255},(SDL_Color){90,120,215,255});
            ...
            renderCenterText(renderer,fSmall,"Built with SDL2 and C  |  2026",
                             cx,698,(SDL_Color){130,150,210,255});
        ...
        /* -------- LOADING -------- */
            ...
            for (int i=0;i<WIN_H;i++) {
                float t=(float)i/WIN_H;
                int rv=(int)(8  + t*155), gv=(int)(15 + t*8), bv=(int)(145-t*90);
                SDL_SetRenderDrawColor(renderer,rv,gv,bv,255);
                SDL_RenderDrawLine(renderer,0,i,WIN_W,i);
            }
            ...
            renderCenterBold(renderer,fHuge,"BISECTION METHOD",         cx, 80,(SDL_Color){255,255,255,255});
            renderCenterBold(renderer,fBig, "QUADRATIC EQUATION",       cx,130,(SDL_Color){255,255,255,255});
            renderCenterBold(renderer,fBig, "ROOT FINDER",              cx,168,(SDL_Color){255,255,255,255});

            /* separator */
            SDL_SetRenderDrawColor(renderer,255,70,70,255);
            SDL_Rect sep={cx-220,214,440,4}; SDL_RenderFillRect(renderer,&sep);

            renderCenterBold(renderer,fBig, "NUMERICAL METHOD  MT 221", cx,228,(SDL_Color){210,225,255,255});
            renderCenterBold(renderer,fBig, "A FINAL PROJECT",          cx,270,(SDL_Color){210,225,255,255});
            renderCenterBold(renderer,fBig, "PROGRAM CREATED BY:",      cx,350,(SDL_Color){255,255,255,255});
            ...
            for (int px=0;px<fillW;px++) {
                ...
                int rv=(int)(18+t*200), gv=(int)(35+t*40), bv=(int)(200-t*160);
                SDL_SetRenderDrawColor(renderer,rv,gv,bv,255);
                SDL_RenderDrawLine(renderer,barX+px,barY,barX+px,barY+barH-1);
            }
            char pct[8];
            sprintf(pct,"%d%%",(int)(progress*100));
            renderCenterBold(renderer,fLarge,pct,cx,barY+11,(SDL_Color){255,255,255,255});

            renderCenterText(renderer,fNorm,"BSCPE 22002",cx,622,
                             (SDL_Color){175,198,255,255});
        }

        /* -------- SOLVER -------- */
        else if (screen==SCREEN_SOLVER) {
            SDL_SetRenderDrawColor(renderer,244,246,255,255);
            SDL_RenderClear(renderer);

            SDL_Color white   = {255,255,255,255};
            SDL_Color cream   = {215,228,255,255};
            SDL_Color blueCol = {12, 42,  150, 255};
            SDL_Color redCol  = {160,18,  18,  255};
            SDL_Color darkTxt = {20, 30,  80,  255};
            SDL_Color hintCol = {95, 120, 185, 255};
            SDL_Color panelBg = {248,250, 255, 255};
            SDL_Color panBdr  = {130,155, 220, 255};
            SDL_Color eqBg    = {238,244, 255, 255};
            SDL_Color eqBdr   = {115,145, 215, 255};

            /* header gradient */
            for (int i=0;i<84;i++) {
                float t=(float)i/84.0f;
                int rv=(int)(8+t*150), gv=(int)(18+t*8), bv=(int)(135-t*60);
                SDL_SetRenderDrawColor(renderer,rv,gv,bv,255);
                SDL_RenderDrawLine(renderer,0,i,WIN_W,i);
            }
            /* red strip */
            SDL_SetRenderDrawColor(renderer,182,18,18,255);
            ...
            /* panel title bar */
            SDL_SetRenderDrawColor(renderer,12,42,150,255);
            ...
            renderBold(renderer,fMed,"f(x) = Ax\xC2\xB2 + Bx + C = 0",40,163,(SDL_Color){155,15,15,255});
            ...
            /* colored header */
            SDL_SetRenderDrawColor(renderer,155,15,15,255);
            ...
            drawPanel(renderer,28,508,464,28,eqBg,eqBdr);
            {
                ...
                renderBold(renderer,fSmall,prev,40,514,(SDL_Color){155,15,15,255});
            }
            ...
            if (!hasSol && !hasError) {
                ...
                if (strlen(inputs[3].value)>0 && strlen(inputs[4].value)>0) {
                    ...
                    SDL_Color hc = (fa2*fb2<0)?(SDL_Color){18,120,40,255}:(SDL_Color){160,18,18,255};
            ...
            if (hasError && strlen(errorMsg)>0) {
                drawPanel(renderer,28,638,464,50,
                    (SDL_Color){255,228,228,255},(SDL_Color){210,60,60,255});
                renderBold(renderer,fSmall,"ERROR:",40,642,(SDL_Color){175,15,15,255});
                renderText(renderer,fSmall,errorMsg,40,659,(SDL_Color){155,20,20,255});
            }

            /* result panel */
            if (hasSol) {
                drawPanel(renderer,28,698,464,108,
                    (SDL_Color){228,238,255,255},(SDL_Color){70,100,210,255});
                ...
                renderText(renderer,fSmall,buf,44,776,(SDL_Color){18,100,40,255});
            ...
            if (hasSol) {
                ...
                renderBold(renderer,fMed,"GIVEN",548,sy+4,redCol);
                ...
                drawPanel(renderer,534,sy,526,dispH,(SDL_Color){240,245,255,255},eqBdr);
                ...
                for (int i=0;i<iterCount;i++) {
                    ...
                    /* row bg */
                    SDL_SetRenderDrawColor(renderer,
                        i%2==0?244:255, i%2==0?247:255, i%2==0?255:255, 255);
                    SDL_Rect rowBg={535,ry-1,524,rowH-1};
                    SDL_RenderFillRect(renderer,&rowBg);

                    /* last row highlight */
                    if (i==iterCount-1) {
                        SDL_SetRenderDrawColor(renderer,215,230,255,255);
                        SDL_RenderFillRect(renderer,&rowBg);
                    }
                ...
                drawPanel(renderer,534,sy,526,56,
                    conv?(SDL_Color){228,255,232,255}:(SDL_Color){255,232,228,255},
                    conv?(SDL_Color){50,160,70,255}:(SDL_Color){200,70,50,255});
                renderBold(renderer,fMed,
                    conv?"CONVERGED - ROOT FOUND":"MAX ITERATIONS REACHED",
                    548,sy+5,
                    conv?(SDL_Color){18,130,38,255}:(SDL_Color){175,30,18,255});
                SDL_SetRenderDrawColor(renderer,
                    conv?50:200, conv?160:70, conv?70:50, 255);
            ...
            for (int di=-4;di<=4;di++) for (int dj=-4;dj<=4;dj++)
                if (di*di+dj*dj<=16) {
                    SDL_SetRenderDrawColor(renderer,20,70,215,255);
                    SDL_RenderDrawPoint(renderer,1127+di,LY+58+dj);
                }
            renderText(renderer,fNorm,"Approximate root (blue dot)",1153,LY+51,(SDL_Color){20,70,215,255});
